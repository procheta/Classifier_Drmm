 ...
 * @author Procheta
 */
public class DownSamplingTopicCreation{

    Properties prop;

    public DownSamplingTopicCreation(Properties prop) throws IOException {
    this.prop = prop;
    }
	...
	fr = new FileReader(new File(prop.getProperty("restFile")));
	...
	while(line != null){
        ...
        bw = new BufferedWriter(fw);
        fr = new FileReader(new File(prop.getProperty("orcasFile")));
        br = new BufferedReader(fr);
        line = br.readLine();
	line = br.readLine();
	...
	fr = new FileReader(new File(prop.getProperty("inputFile")));
	br = new BufferedReader(fr);
	line = br.readLine();
	fw = new FileWriter(new File(prop.getProperty("downSampledId")));
	bw = new BufferedWriter(fw);
	
	while(line != null){
		String st[] = line.split(",");
		bw.write(qMap.get(st[1])+ " "+qMap1.get(st[2])+ " "+st[3]);
		bw.newLine();
		line = br.readLine();
	}
	bw.close();
	...
	fw = new FileWriter(new File(prop.getProperty("downSampledRestId")));
	bw = new BufferedWriter(fw);
	line = br.readLine();
	while(line != null){
		String st[] = line.split(",");
    ...
    public static void main(String[] args) throws FileNotFoundException, IOException, Exception {
        ...
        DownSamplingTopicCreation drp = new DownSamplingTopicCreation(prop);
